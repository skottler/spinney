module Spinney
  module SshCommand
    ...
    def self.included(other)
      other.class_eval do
        deps { Spinney::SshCommand.load_deps } unless @dependency_loader
    ...
    def sudo_available?
      ...
      puts "`sudo` not available on #{host}" unless @sudo_available
      @sudo_available
    end

    def process_sudo(command)
      if sudo_available?
        replacement = 'sudo -p \'sudo password: \''
      else
        replacement = ''
      end
    ...
    def run_command(command, options={})
      ...
      puts "Running processed command #{command}"
